...
update_apt() {
    ...
    ssh -o StrictHostKeyChecking=no -i "$ssh_keyfile" -p "${host##*:}" "$ssh_user@${host%:*}" sudo apt-get update
...
upgrade_packages() {
    ...
    ssh -o StrictHostKeyChecking=no -i "$ssh_keyfile" -p "${host##*:}" "$ssh_user@${host%:*}" sudo apt-get upgrade -y
