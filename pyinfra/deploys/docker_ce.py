...
from pyinfra.operations import apt, dnf, files, yum, server
...
def _apt_install():
    ...
    apt.packages(
        ...
        present=True,  # update if we added the repo
    )
...
def _yum_or_dnf_install(yum_or_dnf):
    ...
    yum_or_dnf.packages(
        name="Install Docker Compose via yum",
        packages=["docker-compose-plugin"],
        present=True,
    )
...
def deploy_docker(config=None):
    ...
    docker_user = os.environ.get("ZUSER")
    server.shell(
        name="Add to docker group",
        commands=[f"sudo usermod -aG docker {docker_user}"],
    )
